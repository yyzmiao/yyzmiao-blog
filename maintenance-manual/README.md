...
每篇文章填写一个一级分类和一个二级分类。网站会在顶部分类栏、侧栏分类树和分类总览页自动生成入口。
...
- `技术`：`网络与安全`、`系统与运维`、`开发与编程`、`移动设备`、`AI 与自动化`
- `项目`：`自研项目`、`开源改造`
- `资源`：`软件工具`、`配置模板`、`资料整理`
- `随笔`：`站点记录`、`日常想法`

分类目录统一维护在 `src/config/categoryConfig.ts`。分类尽量保持稳定；标签用于描述更具体的技术、产品和场景。一篇文章只能有一组一、二级分类，但可以填写多个标签。
